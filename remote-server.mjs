...
function normalizeCategoryLabel(category) {
  ...
  if (value === "模型训练") return "模型判断";
  return value;
}

function inferModuleAndCategory(rawModule, rawCategory) {
  const moduleValue = String(rawModule || "").trim();
  const categoryValue = normalizeCategoryLabel(rawCategory);

  if (ALL_MODULES.includes(moduleValue)) {
    const subCats = MODULE_MAP[moduleValue] || [];
    if (subCats.length === 0) {
      return { module: moduleValue, category: null };
    }
    return { module: moduleValue, category: categoryValue };
  }

  if (categoryValue && categoryValue.startsWith("项目")) {
    return { module: "项目", category: null };
  }

  return { module: "笔记", category: categoryValue };
}

function articleHasExpiringNotionImages(article) {
  const html = String(article?.contentHtml || "");
  return /prod-files-secure\.s3\.[^"' ]+amazonaws\.com/i.test(html) || /X-Amz-Algorithm=AWS4-HMAC-SHA256/i.test(html);
}

async function rebuildArticleFromNotion(article) {
  const pageId = article?.notionPageId || extractNotionPageId(article?.notionUrl || "");
  ...
  return {
    ...
    slug: slugify(title || article.title),
    notionPageId: pageId,
    contentHtml,
...
function normalizeArticle(article) {
  if (article.module) {
    return {
      ...article,
      category: normalizeCategoryLabel(article.category)
    };
  }
  ...
  const oldMap = {
    ...
    "模型训练": { module: "笔记", category: "模型判断" }
  };
  const mapped = oldMap[article.category] || { module: "笔记", category: article.category };
  return { ...article, module: mapped.module, category: normalizeCategoryLabel(mapped.category) };
}
...
async function handleNotionImport(req, res) {
  ...
  const selection = inferModuleAndCategory(body.module, body.category);
  ...
  const finalCategory = subCats.length > 0 ? normalizeCategoryLabel(category) : null;
...
async function handleNotionArticleGet(req, res, articleId) {
  ...
  const article = index >= 0 ? store.articles[index] : null;
  ...
  let current = article;
  if (articleHasExpiringNotionImages(article) && NOTION_API_KEY) {
    try {
      current = await rebuildArticleFromNotion(article);
      store.articles[index] = current;
      await writeArticleStore(store);
    } catch (error) {
      console.warn(`refresh notion article failed: ${articleId}: ${error.message}`);
    }
  }

  const norm = normalizeArticle(current);
  json(res, 200, {
    ok: true,
    article: {
      id: norm.id,
